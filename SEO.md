...
- `seo.indexable` è `true` per il lancio autorizzato dal gestore. Per una build di anteprima impostarlo a `false`; il dominio da solo non abilita l’indicizzazione.
- In anteprima il `noindex` è leggibile perché `robots.txt` permette la scansione. Il file robots non è una protezione per dati riservati.
- Per la pubblicazione, completare tutti i riferimenti visibili in `site`, verificare telefono internazionale, email e link HTTPS, quindi impostare `seo.indexable: true`. La build segnala i campi mancanti e fallisce se si tenta di indicizzare i placeholder. La verifica comprende `src/privacy.config.js`: dati effettivi completi, `reviewed: true` e `requestRetentionConfirmed: true`. Lo stesso controllo determina la presenza dell’avviso di bozza nella pagina privacy. Il modulo prepara esclusivamente un’email da inviare dal programma di posta dell’ospite.
...
2. Eseguire `npm run build` e `npm test`: i controlli coprono anteprima, pubblicazione, URL, sitemap, metadati, riferimenti JSON-LD e browser desktop/mobile. I test unitari coprono anche dati simulati e modalità anteprima; i test browser verificano la build pubblica effettiva.
